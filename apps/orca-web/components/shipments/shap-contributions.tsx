  ...
  return (
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 flex items-center gap-2">
        <div className="flex-1 h-3 rounded-full bg-slate-100 relative overflow-hidden">
          <div
            className={`absolute top-0 h-full rounded-full transition-all ${
              isPositive ? "bg-red-400 left-0" : "bg-blue-400 right-0"
            }`}
            style={{ width: `${pct * 100}%` }}
          />
        </div>
        <span className={`text-xs font-mono w-14 text-right ${isPositive ? "text-red-600" : "text-blue-600"}`}>
          {contribution >= 0 ? "+" : ""}
          ...
          <div>
            <div className="flex text-xs text-slate-400 mb-2 gap-3">
              ...
              <span className="flex-1">
                <span className="text-red-400">red = increases risk</span>
                {" / "}
                <span className="text-blue-400">blue = reduces risk</span>
              </span>
